...
const CustomSelect = ({
  ...
  // 🔑 Ref to always hold the latest closeMenu function
  ...
  // Sync search text with single-select value
  useEffect(() => {
    if (!isMulti && !isSearchable) {
      setSearchText(selected ? getOptionLabel(selected) : "");
    }
  }, [selected, isMulti, isSearchable]);
  ...
  const closeMenu = useCallback(() => {
    setIsOpen((prev) => {
      ...
      if (!isMulti && !isSearchable) {
        setSearchText(selected ? getOptionLabel(selected) : "");
      } else if (!isMulti && isSearchable) {
        setSearchText("");
      }
      onMenuClose?.();
      return false;
    });
  }, [isMulti, isSearchable, selected, onMenuClose]);

  // 🔑 Keep the ref pointing to the latest closeMenu
  useEffect(() => {
    closeMenuRef.current = closeMenu;
  }, [closeMenu]);

  // 🔑 Outside click — uses the ref so it's never stale
  useEffect(() => {
    const handler = (e) => {
    ...
    // Use mousedown for faster response, but capture phase to beat other handlers
  ...
  const handleSelect = (option) => {
    ...
    if (isMulti) {
      ...
      emitChange(option);
      setSearchText(isSearchable ? "" : getOptionLabel(option));
      closeMenu();
    }
  };
